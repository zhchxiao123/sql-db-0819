...
#include "slt_odbc3.c"
#endif
#include "slt_subprocess.c"   /* sql-db-0819 engine driver (see PIN.txt) */
#ifdef OMIT_SQLITE
/* The runner uses sqlite3_snprintf() to render hash summaries; without the
** embedded SQLite engine (and its header) provide an equivalent shim.
** Note the argument order differs from snprintf(): (int n, char *zBuf, fmt). */
#include <stdarg.h>
static void sqlite3_snprintf(int n, char *zBuf, const char *zFormat, ...){
  va_list ap;
  va_start(ap, zFormat);
...
int main(int argc, char **argv){
  ...
  */
  ...
  registerSqlite();
#endif
  registerSubprocess();   /* sql-db-0819 engine driver (see PIN.txt) */
#ifndef OMIT_ODBC
  registerODBC3();
#endif
